...
public class BwaInterpreter {
	...
	private JavaRDD<Text> handleSingleReadsSorting() {
        ...
        JavaPairRDD<LongWritable, Text> singleReadsKeyVal = ctx.newAPIHadoopFile(options.getInputPath(), FastqInputFormat.class, LongWritable.class, Text.class, this.conf);
	...
	private JavaRDD<Tuple2<Text,Text>> handlePairedReadsSorting() {
        ...
        // Read the two FASTQ files from HDFS using the FastqInputFormat class
        JavaPairRDD<LongWritable,Text> datasetTmp1 = ctx.newAPIHadoopFile(options.getInputPath(), FastqInputFormat.class, LongWritable.class, Text.class, this.conf);
        JavaPairRDD<LongWritable,Text> datasetTmp2 = ctx.newAPIHadoopFile(options.getInputPath2(), FastqInputFormat.class, LongWritable.class, Text.class,this.conf);
